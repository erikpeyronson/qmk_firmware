...
static int my_td_current_dance(tap_dance_state_t *state)
{
  if (state->count == 1)
    ...
    {
      /*
       * DOUBLE_SINGLE_TAP is to distinguish between typing "pepper", and actually wanting a double
       * tap action when hitting 'pp'. Suggested use case for this return value is when you want to
       * send two keystrokes of the key, and not the 'double tap' action/macro.
       */
  ...
  // If your tap dance key is 'KC_W', and you want to type "www." quickly - then you will need to
  // add an exception here to return a 'TRIPLE_SINGLE_TAP', and define that enum just like
  // 'DOUBLE_SINGLE_TAP'
  if (state->count == 3)
    {
